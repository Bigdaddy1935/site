...
const TextInput = React.forwardRef((props: Props, ref) => {
  ...
  const { field, className, multiple, Icon, error, ...allProps } = props;

  const inputProps = {
    className: twMerge(
      error ? "!outline-rose-700" : "",
      `bg-hgray-200 dark:bg-mdark-600 dark:text-white w-full p-2 rounded-lg outline outline-2 outline-hgray-300 dark:outline-mdark-400 focus:outline-primary-400 dark:bg-mdark-500`,
      className
    ),
    ...allProps,
    ...field,
    ...
    <div className="relative">
      ...
      {Icon ? (
        <Icon
          width={24}
          height={24}
          className="text-primary-400 dark:text-primary-100 absolute top-2 left-2"
        />
      ) : null}
    </div>
  );
