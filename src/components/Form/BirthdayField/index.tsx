...
export default function BirthdayField(props: Props) {
  const { name, label, className, pattern, row, wrapperClassName, required } =
    props;
  ...
  return (
    <Controller
      ...
      render={({ field, fieldState: { error } }) => (
        <div
          className={twMerge(
            `flex dark:bg-mdark-500 ${row ? "items-center justify-center" : "flex-col items-stretch"} gap-1 lg:gap-3 lg:flex-row lg:items-center lg:min-w-[365px] w-full  mb-3`,
            wrapperClassName,
        ...
        >
          ...
          <div className={`${!row && "flex-1"} relative`}>
            <NumberFormatBase
              className={twMerge(
                "bg-hgray-200 dark:bg-mdark-500 p-2 text-hgray-400 dark:text-hgray-200  rounded-lg !border-none !shadow-none !outline-0 text-right",
                className
              )}
              dir="ltr"
              format={_format}
