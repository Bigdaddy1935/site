...
import { useForgotPasswordMutation, usePhoneCheckMutation } from "@/lib/services/auth";
...
const text: KeyValue = {
    '/user/check-user': ' برای ورود و یا ثبت نام، اطلاعات کاربری خود را وارد کنید.',
    '/user/forget-password': 'جهت بازیابی رمز عبور اطلاعات کاربری خود را وارد کنید.'
}
export default function CheckUserForm() {
    const [checkPhone, { isLoading, isSuccess, data }] = usePhoneCheckMutation();
    const [forgetPassword, { isLoading: forgetPasswordLoading, isSuccess: forgetPasswordSuccess }] = useForgotPasswordMutation();
    ...
    const handleForgetPassword = (values: any) => {
        forgetPassword({ phone: values.phone.replaceAll(' ', '') }).then(({ error }) => {
            if (!error) {
    ...
    const handleCheckUser = (values: any) => {
        checkPhone({ phone: values.phone.replaceAll(' ', '') }).then(({ data, error }) => {
            if (error) return;
            dispatch(setRedirect({
                phone: values.phone.replaceAll(' ', ''),
                action: data.UserExisted ? 'login' : 'signup'
            }));
            redirectWithQueryString(data?.UserExisted ? '/user/password-login' : '/user/create-acount');
        })
    }
    const handleSubmit = (values: any) => {
        action === 'reset-password' ? handleForgetPassword(values) : handleCheckUser(values);
    }
    return (
        <div className={`p-6 lg:max-w-[396px] mx-auto`}>
            ...
            <FormProvider {...methods}>
                <form className="flex flex-col lg : items-end gap-y-3" onSubmit={methods.handleSubmit(handleSubmit)}>
                    ...
                    <SubmitButton type="submit" disabled={isLoading || isSuccess || forgetPasswordLoading || forgetPasswordSuccess}>
                        {isLoading || forgetPasswordLoading ? <IconLoading className="mx-auto" width={24} height={24} /> :
                            <>
                                {"ادامه"}
                            </>
                        }
...
function SubmitButton(props: SubmitButtonProps) {
    ...
    return (
        <button
            ...
            className={clsx(`w-full lg:mt-3  font-medium  rounded-md p-2 cursor-pointer bg-primary-300  text-white dark:disabled:text-hgray-300`, className)}
            {...allProps}
        >
            {children}
        </button>
