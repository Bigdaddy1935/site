...
import { useCheckTokenMutation, useLoginTokenMutation } from '@/lib/services/auth';
...
export default function VerificationForm() {
  ...
  const phone = useAppSelector(selectPhone) ?? '09356867222';
  ...
  const [loginToken, { isLoading: loginTokenLoading }] = useLoginTokenMutation();
  const [checkToken, { isLoading: checkTokenLoading }] = useCheckTokenMutation();
  ...
  const handleCheckToken = () => {
    checkToken({ phone, token: verifyCode }).then(({ data, error }) => {
      if (!error) {
        toast.success('کد احراز هویت شما تایید شد.');
        redirectWithQueryString(action === 'signup' ? '/user/signup' : '/user/reset-password');
      }
    });
  };
  const handleSubmit = () => {
  ...
  return (
    <div className="flex flex-col justify-center gap-5">
      ...
      <LoadingButton
        ...
        className="w-full rounded-lg bg-white p-4 text-base text-hgray-600 dark:text-text-dark-2 dark:bg-mdark-400"
      >
        تایید
      </LoadingButton>

      <p className="text-center text-sm text-hgray-400 dark:text-text-dark-2">
        در صورتی که پیامکی به شما ارسال نشده میتوانیداز ارسال مجدد استفاده نمایید
      </p>
