    ...
    <div>
      ...
      <div className="flex flex-col flex-wrap justify-between gap-[2%] gap-y-[2vw] lg:flex-row">
        {items.map(({ IconComponent, label, text, color }) => (
          <Paper
            key={text}
            className="flex w-full flex-1 justify-between lg:min-w-[49%] lg:items-center"
          >
            <span style={{backgroundColor : `${color}52`}} className="relative ml-4 h-14 w-14 rounded-full bg-primary-100/65 dark:bg-primary-700/40">
              <IconComponent
                style={{ color }}
